...
function Player(pos) {
  ...
  this.size = new Vector(0.8, 1.5);   //edit this portion if you want to change characters size
...
/**function Lava(pos, ch) {
  ...
  if (ch == "=") {									//horiz lava speed
    // Horizontal lava
    this.speed = new Vector(2, 0);					//other lava speeds********************************
  } else if (ch == "|") {
    // Vertical lava
...
/**
player.prototype.act = function(step) {
  var wobbleSpeed = 8, wobbleDist = 0.07;
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
		...
		alert("Geronimo!!! We've done it! HA!");
        console.log("You win!");
    });
  }
  startLevel(0);
